...
def dijkstra_lite(graph: Graph_lite, src: int) -> list[list[float | int]]:
    distances: list[list[float | int]] = [[inf, 0] for _ in range(graph.n + 1)]
...
def unused_connections(
    graph: Graph_lite, distances: list[list[float | int]]
) -> list[list[tuple[int, float]]]:
    adj = [list(edges) for edges in graph.adj]
    for i in range(1, len(distances)):
        source = i
        target = int(distances[i][1])
        if target == -1:
            continue
